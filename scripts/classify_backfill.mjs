...
//
// 필요 env: SUPABASE_URL, SUPABASE_SERVICE_KEY, GOOGLE_AI_API_KEY(분류 시).
// =====================================================================
...
// Google AI Studio 키(GEMINI_API_KEY 별칭 허용)
const LLM_KEY = process.env.GOOGLE_AI_API_KEY || process.env.GEMINI_API_KEY || '';
// Gemini 2.5는 기본 사고 활성 → 작은 출력 상한을 사고 토큰이 소진하지 않도록 기본 0(비활성).
const THINKING_BUDGET =
  process.env.GEMINI_THINKING_BUDGET === undefined ? 0 : Number(process.env.GEMINI_THINKING_BUDGET);
...
function requireEnv() {
  ...
  if (!LLM_KEY) console.warn('[WARN] GOOGLE_AI_API_KEY 없음 → LLM 분류 불가(후보는 보류 처리)');
}

// Gemini generateContent + JSON 파싱 (재시도 1회). 실패 시 null.
async function geminiJson(sys, user, maxTokens) {
  const useThinking =
    Number.isFinite(THINKING_BUDGET) &&
    THINKING_BUDGET >= 0 &&
    !(THINKING_BUDGET === 0 && /pro/i.test(CLASSIFY_MODEL));
  const body = {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      const res = await fetch(
        `${GOOGLE_AI_BASE_URL}/models/${encodeURIComponent(CLASSIFY_MODEL)}:generateContent`,
        {
          method: 'POST',
...
async function verifyLLM(b) {
  ...
  const o = await geminiJson(sys, user, 120);
  if (o && typeof o.keep === 'boolean') return o.keep;
  return true; // 검증 실패 → 보수적 유지
}
